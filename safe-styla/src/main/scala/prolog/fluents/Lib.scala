...
S:length(Xs,L):-nonvar(L),!,len2list(L,Xs).
S:length(Xs,L):-list2len(Xs,L).
...
S:('\=='(X,Y)):-S:(term_compare(X,Y,R)),S:(R=\=0).
'@>'(X,Y):-term_compare(X,Y,1).
'@>='(X,Y):- \+('@<'(X,Y)).
'@=<'(X,Y):- \+('@>'(X,Y)).
...
S:last([X],Last):-!,Last=X.
S:last([_|Xs],Last):-S:last(Xs,Last).
